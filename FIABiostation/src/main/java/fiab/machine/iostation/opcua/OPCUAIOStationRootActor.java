...
public class OPCUAIOStationRootActor extends AbstractTracingActor {
	...
	static public Props propsForInputStation(String machineName, int portOffset) {
		return Props.create(OPCUAIOStationRootActor.class,
				() -> new OPCUAIOStationRootActor(machineName, portOffset, true));
	}

	static public Props propsForOutputStation(String machineName, int portOffset) {
		return Props.create(OPCUAIOStationRootActor.class,
				() -> new OPCUAIOStationRootActor(machineName, portOffset, false));
	}
	...
	private void init(int portOffset) throws Exception {
		...
		fu = isInputStation
				? new IOStationHandshakeFU.InputStationHandshakeFU(opcuaBase, ioNode, fuPrefix, getSelf(), getContext(),
						"DefaultServerSideHandshake", OPCUACapabilitiesAndWiringInfoBrowsenames.IS_PROVIDED, true)
				: new IOStationHandshakeFU.OutputStationHandshakeFU(opcuaBase, ioNode, fuPrefix, getSelf(),
						getContext(), "DefaultServerSideHandshake",
		...
		CapabilityExposingUtils.setupCapabilities(opcuaBase, ioNode, fuPrefix,
				new CapabilityImplementationMetadata("DefaultStation",
						isInputStation ? IOStationCapability.INPUTSTATION_CAPABILITY_URI
								: IOStationCapability.OUTPUTSTATION_CAPABILITY_URI,
						ProvOrReq.PROVIDED));
		Thread s1 = new Thread(opcuaBase);
		s1.start();
		/*
		 * if (doAutoReload) { reloadPallet(); //We will use hardware for this }
		 */
	}
	...
	private void setupOPCUANodeSet(OPCUABase opcuaBase, UaFolderNode folderNode, String path, ActorRef actor) {
		...
		// if (IOStationCapability.STATE_VAR_NAME !=
		// IOStationCapability.OPCUA_STATE_SERVERSIDE_VAR_NAME) {
		statusIOS = opcuaBase.generateStringVariableNode(folderNode, path, IOStationCapability.STATE_VAR_NAME,
				ServerSideStates.STOPPED);
		// }
	}
