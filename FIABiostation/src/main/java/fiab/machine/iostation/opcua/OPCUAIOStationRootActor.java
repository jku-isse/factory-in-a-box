...
public class OPCUAIOStationRootActor extends AbstractActor {
    ...
    static public Props propsForInputStation(String machineName, int portOffset, EV3ColorSensor colorSensor, boolean isInputStation, boolean doAutoReload) {
        return Props.create(OPCUAIOStationRootActor.class, () -> new OPCUAIOStationRootActor(machineName, portOffset, colorSensor, isInputStation, doAutoReload));
    }

    static public Props propsForOutputStation(String machineName, int portOffset, EV3ColorSensor colorSensor, boolean isInputStation, boolean doAutoReload) {
        return Props.create(OPCUAIOStationRootActor.class, () -> new OPCUAIOStationRootActor(machineName, portOffset, colorSensor, isInputStation, doAutoReload));
    }

    public OPCUAIOStationRootActor(String machineName, int portOffset, EV3ColorSensor colorSensor, boolean isInputStation, boolean doAutoReload) {
        try {
            this.machineName = machineName;
            this.isInputStation = isInputStation;
            this.doAutoReload = doAutoReload;
            this.colorSensor = colorSensor;
            log.info("ColorSensor=" + colorSensor);
            this.self = self();
            init(portOffset);
        } catch (Exception e) {
            e.printStackTrace();
