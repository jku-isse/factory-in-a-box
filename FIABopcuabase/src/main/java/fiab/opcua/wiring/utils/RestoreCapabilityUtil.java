...
public class RestoreCapabilityUtil {
...
//									"RemoteRole1");
        ...
        try {
            new RestoreCapabilityUtil().restoreOriginalWiringInfo(endpointUrl, "NORTH_CLIENT", northWI,"EAST_CLIENT", eastWI);
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public void restoreOriginalWiringInfo(String endpointUrl, String capNorthWiring, WiringInfo wiNorth, String capEastWire, WiringInfo wiEast) throws Exception {
        // restore north cap wiring
        OpcUaClient oldClient = new OPCUAClientFactory().createClient(endpointUrl);
        oldClient.connect().get();
        new CapabilityDiscovery(endpointUrl, oldClient).discoverAll().stream().filter(cii -> cii.metaData.getImplId().equals(capNorthWiring)).findAny().ifPresent(
                cii -> executeWiringInformation(cii, wiNorth)
        );
