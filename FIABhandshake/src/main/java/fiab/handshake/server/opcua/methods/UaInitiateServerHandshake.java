...
public class UaInitiateServerHandshake extends AbstractMethodInvocationHandler {

    private final Logger logger = LoggerFactory.getLogger(getClass());
    private final Duration timeout = Duration.ofSeconds(2);
    private final ActorRef actor;

    public UaInitiateServerHandshake(UaMethodNode node, ActorRef actor) {
        super(node);
        this.actor = actor;
    }
    ...
    protected Variant[] invoke(InvocationContext invocationContext, Variant[] inputValues) throws UaException {
        ...
        try {
            resp = ((ServerHandshakeResponseEvent) ask(actor,
                    new InitiateHandoverRequest(invocationContext.getObjectId().toParseableString()), timeout)
