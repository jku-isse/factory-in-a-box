...
public class OrderProcess {
	...
	private void makeNextStepsAvailable(ProcessStep step, ProcessChangeImpact pci) {
		// makes step only available if in state initiated, then also recursively continue, and only if prior ones are completed or canceled
		...
		if (step instanceof ProcessCore.Process)
			((ProcessCore.Process) step).getSteps().stream().findFirst().ifPresent(childStep -> makeNextStepsAvailable(childStep, pci));
		if (step instanceof ParallelBranches) {
	...
	private void propagateCompletionToParent(ProcessStep step, ProcessChangeImpact pci) {
		...
		if (parentAsO != null && parentAsO instanceof ProcessStep) {			
			...
			if (areAllChildrenOfProcessCancelledOrCompleted((ProcessStep)parent)) {
				// if all child steps complete or canceled propagate up
				stepStatus.put(step, StepStatusEnum.COMPLETED);	
				pci.markStepInNewState(StepStatusEnum.COMPLETED, (ProcessStep) parent);
				propagateCompletionToParent((ProcessStep) parent, pci);
			} else { // not all substeps are complete or canceled
				makeNextStepsAvailable((ProcessStep) parent, pci);
			}
		}
	}
	...
	private boolean areAllChildrenOfProcessCancelledOrCompleted(ProcessStep step) {
		if (step instanceof ProcessCore.Process)
			((ProcessCore.Process) step).getSteps().stream().allMatch(childStep -> {
		...
		if (step instanceof ParallelBranches) {
			((ParallelBranches)step).getBranches().stream().allMatch(childStep -> { 
					StepStatusEnum status = stepStatus.getOrDefault(childStep, StepStatusEnum.INITIATED);
					return status.equals(StepStatusEnum.COMPLETED) || status.equals(StepStatusEnum.CANCELED);	
				} );
		} 
