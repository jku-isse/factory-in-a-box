...
import fiab.mes.layoutAgent.msg.GetLayoutRequest;
import fiab.mes.layoutAgent.msg.GetLayoutResponse;
...
import fiab.mes.transport.actor.transportsystem.TransportModuleUsageTracker;
...
public class AdaptedTransportSystemCoordinatorActor extends AbstractActor {
	...
	static public Props props(TransportRoutingInterface routing, TransportPositionLookupInterface dns, int expectedTTs, String lookupPrefix) {
		return Props.create(AdaptedTransportSystemCoordinatorActor.class, () -> new AdaptedTransportSystemCoordinatorActor(routing, dns, expectedTTs, lookupPrefix));
	}

	static public Props props(TransportRoutingInterface routing, TransportPositionLookupInterface dns, int expectedTTs) {
		return Props.create(AdaptedTransportSystemCoordinatorActor.class, () -> new AdaptedTransportSystemCoordinatorActor(routing, dns, expectedTTs));
	}
	...
	public AdaptedTransportSystemCoordinatorActor(TransportRoutingInterface routing, TransportPositionLookupInterface dns, int expectedTTs, String lookupPrefix) {
	...
	public AdaptedTransportSystemCoordinatorActor(TransportRoutingInterface routing, TransportPositionLookupInterface dns, int expectedTTs) {
		this.routing = routing;
		this.dns = dns;
		this.self = getSelf();
		this.externalHistory = new HistoryTracker(WELLKNOWN_LOOKUP_NAME);
	...
	public Receive createReceive() {
		return receiveBuilder()
				...
				.match(RealTimeShopfloorLayout.class, layout -> {
					log.info("Updating Shopfloor Layout: {}", layout);
					this.routing = layout.transportRouting;
					this.dns = layout.positionLookup;
					this.tmut = new TransportModuleUsageTracker();
					this.expectedTTs = layout.transportUnitCount;
				})
	...
	private void handleCancelTransportRequest(CancelTransportRequest req) {
		...
		tmrOp.ifPresent(tmr -> { requestQueue.remove(tmr); 
			...
			log.info(msg);
			rtr.getRequestor().tell(new RegisterTransportRequestStatusResponse(rtr, ResponseType.CANCELED ,msg), self);
		});
		...
		Optional<TransportModuleRequest> tmrOp2 = allocatedQueue.stream()
			...
			tmrOp2.ifPresent(tmr -> { 
				...
				log.warning(msg);
				rtr.getRequestor().tell(new RegisterTransportRequestStatusResponse(rtr, ResponseType.ABORTED ,msg), self);
			});
	}
		...
		prevValues.ifPresent(prevOR -> {
			...
			if ( prevOR.getKey() != null &&				
				 tmut.getTransportModulesInvolvedInOrder(prevOR.getKey()).size() == 0) {
					...
					if (rtr != null)
						rtr.getRequestor().tell(new RegisterTransportRequestStatusResponse(rtr, ResponseType.COMPLETED ,msg), self);
			...
			try {
				...
				case 3: 					
					...
					if (act1_3.isPresent()) {
						tmrRoot = act1_3.map(actor -> { if (tmut.isTransportModule(actor)) { // must be a turntable
							...
							log.warning(msg);
							this.getSender().tell(new RegisterTransportRequestStatusResponse(rtr, ResponseType.MISSING_TRANSPORT_MODULE ,msg), self);
							return null;
						}
						});
					} else {
				...
				case 4:	// same as for case 3 but another stop in between, 
					...
					if (!act1_4.isPresent()) {
						...
						log.warning(msg);
						this.getSender().tell(new RegisterTransportRequestStatusResponse(rtr, ResponseType.UNSUPPORTED_TRANSIT_POSITION ,msg), self);
						break;
					}
					if (!act2_4.isPresent()) {
				...
				default:
					...
					log.warning(msg);					
					this.getSender().tell(new RegisterTransportRequestStatusResponse(rtr, ResponseType.UNSUPPORTED_ENDPOINT_POSITIONS ,msg), self);
					break;
				}
				tmrRoot.ifPresent(tmr -> {
					...
					this.getSender().tell(new RegisterTransportRequestStatusResponse(rtr, ResponseType.QUEUED ,msg), self);
					log.info(msg);
				...
				log.warning(msg);
				this.getSender().tell(new RegisterTransportRequestStatusResponse(rtr, ResponseType.NO_ROUTE ,msg), self);
			}
		} else {
