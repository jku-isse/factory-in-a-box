...
public class AdaptedOrderPlanningActor extends AbstractActor {
    ...
    protected MachineCapabilityManager capMan;
    ...
    static public Props props() {
        return Props.create(AdaptedOrderPlanningActor.class, () -> new AdaptedOrderPlanningActor());
    }

    public AdaptedOrderPlanningActor() {
        this.self = getSelf();
        this.capMan = new MachineCapabilityManager();
        try {
    ...
    public Receive createReceive() {
        return receiveBuilder()
                ...
                .match(OrderRelocationNotification.class, msg -> {
                    log.info("Received Order Relocation Notification with order id " + msg.getOrderId() +
...
//						assign inputstation 
    ...
    private void doOrderRelocationFromToMachine(String orderId, AkkaActorBackedCoreModelAbstractActor destination) {
        // get actor from id as source
        ordMapper.updateOrderLocation(orderId, destination);
    }
    ...
    private void handleCancelOrderRequest(CancelOrTerminateOrder req) {
        ...
        ordMapper.getLastOrderState(req.getRootOrderId()).ifPresent(oStatus -> {
            ...
            switch (oStatus) {
                ...
                case COMPLETED:
                    log.info("Process %s is waiting for transport to output station", req.getRootOrderId());
                ...
                case PREMATURE_REMOVAL:
                    log.warning("Order %s did not complete due to status %s", req.getRootOrderId(), oStatus);
                    // nothing to do
                    break;
