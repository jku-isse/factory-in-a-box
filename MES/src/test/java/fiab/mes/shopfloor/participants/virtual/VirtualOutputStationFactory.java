...
public class VirtualOutputStationFactory {
     ...
     */
    public static ParticipantInfo initOutputStationWithProxyAndReturnInfo(ActorSystem system, String machineId, PositionMap positionMap) {
        int opcUaPort = positionMap.get(machineId).getOpcUaPort();
        Position position = positionMap.getPositionForId(machineId);
        ActorRef remoteMachine = OutputStationFactory.startStandaloneOutputStation(system, opcUaPort, machineId);
     ...
     * @return model actor
     */
    private static Actor createParticipantModelActor(String machineId, int opcUaPort) {
        ...
        actor.setUri(localhostOpcUaPrefix + opcUaPort + "/" + machineId);
        return actor;
    }
}
