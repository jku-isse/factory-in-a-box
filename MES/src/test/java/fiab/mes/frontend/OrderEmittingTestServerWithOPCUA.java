        ...
        new TestKit(system) {
            {
                    ...
                    if (te instanceof MachineStatusUpdateEvent) {
                        if (((MachineStatusUpdateEvent) te).getStatus().equals(BasicMachineStates.STOPPED))
                            Optional.ofNullable(knownActors.get(((MachineStatusUpdateEvent) te).getMachineId()))
                                    .ifPresent(actor -> actor.getAkkaActor().tell(new GenericMachineRequests.Reset(((MachineStatusUpdateEvent) te).getMachineId()),
                                            getRef())
                                    );
                        if (((MachineStatusUpdateEvent) te).getStatus().equals(BasicMachineStates.COMPLETE)) {
                            Optional.ofNullable(knownActors.get(((MachineStatusUpdateEvent) te).getMachineId()))
                                    .ifPresent(actor -> {
                                        if (actor.getId().equals("OutputStation")) {
                                            System.out.println("OutputStation has completed");
                                        } else {
                                            System.out.println("Machine with id=" + actor.getId() + " has completed");
                                        }
                                    });
                        }
                    }
                ...
                Assertions.assertNotEquals(1000, retval);    //retval is between -1 to 255, we just let junit wait until the test has completed
