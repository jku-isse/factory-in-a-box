...
public class ProduceProcess {
    ...
    public static ProcessCore.Process getSingleFoldBoxProcess(String prefix) {
        return getSequentialFoldProcess(prefix, ProcessCoreFactory.eINSTANCE.createXmlRoot());
    }

    public static ProcessCore.Process getSequentialFoldProcess(String prefix, XmlRoot root) {
        // first four supported product: BOX
        ProcessCore.Process p = ProcessCoreFactory.eINSTANCE.createProcess();
        p.setDisplayName(prefix + "SequentialProcess");
        p.setID(prefix + "SequentialProcess");

        AbstractCapability cap = WellknownFoldingCapability.getFoldingShapeCapability();//WellknownPlotterCapability.getColorPlottingCapability(color);
        root.getCapabilities().add(cap);
        ...
        s1.setInvokedCapability(root.getCapabilities().get(0));
        s1.getInputMappings().add(EcoreProcessUtils.getVariableMapping(root.getCapabilities().get(0).getInputs().get(0)));
        EcoreProcessUtils.addProcessvariables(p, "box");
        EcoreProcessUtils.mapCapInputToProcessVar(p.getVariables(), s1);
        p.getSteps().add(s1);
