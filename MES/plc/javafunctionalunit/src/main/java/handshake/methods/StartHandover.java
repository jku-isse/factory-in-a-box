...
public class StartHandover extends AbstractMethodInvocationHandler {
    ...
    protected Variant[] invoke(InvocationContext invocationContext, Variant[] inputValues) throws UaException {
    	...
    	Object resp;
		try {
			resp = Patterns.ask(actor, HandshakeProtocol.ServerMessageTypes.RequestStartHandover, timeout).toCompletableFuture().get();
		} catch (InterruptedException | ExecutionException e) {
			logger.error(e.getMessage());
			resp = HandshakeProtocol.ServerMessageTypes.NotOkResponseStartHandover;
		}    	        
