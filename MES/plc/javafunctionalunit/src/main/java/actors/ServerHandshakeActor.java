...
public class ServerHandshakeActor extends AbstractActor{

	private LoggingAdapter log = Logging.getLogger(getContext().getSystem(), this);	
	protected boolean isLoaded = false; //assume at bootup that no pallet is loaded
	private ActorRef machineWrapper;
	protected HandshakeProtocol.ServerSide currentState = HandshakeProtocol.ServerSide.STOPPED;
	...
	static public Props props(ActorRef machineWrapper, boolean doAutoComplete) {	    
		return Props.create(ServerHandshakeActor.class, () -> new ServerHandshakeActor(machineWrapper, doAutoComplete, null));
	}
	
	static public Props props(ActorRef machineWrapper, boolean doAutoComplete, StatePublisher publishEP) {	    
		return Props.create(ServerHandshakeActor.class, () -> new ServerHandshakeActor(machineWrapper, doAutoComplete, publishEP));
	}
	
	public ServerHandshakeActor(ActorRef machineWrapper, boolean doAutoComplete, StatePublisher publishEP) {
	...
	public Receive createReceive() {
		return receiveBuilder()
				.match(HandshakeProtocol.ServerMessageTypes.class, msg -> {
					...
					switch(msg) {					
					case Complete:
						if (currentState.equals(HandshakeProtocol.ServerSide.EXECUTE)) {
							complete();
						}
						break;
					...
					case RequestStartHandover:
						...
						break;
					case Reset:
						if (currentState.equals(HandshakeProtocol.ServerSide.STOPPED) || currentState.equals(HandshakeProtocol.ServerSide.COMPLETE)) {
							reset();
						}
						break;
					case Stop:
	...
	protected void publishNewState(HandshakeProtocol.ServerSide newState) {
	...
	protected void updateLoadState(boolean isLoaded) {
		if (this.isLoaded != isLoaded) {
			...
			if (currentState != HandshakeProtocol.ServerSide.STOPPED)
				stop();
		}
	}
	
	protected void reset() {;
		publishNewState(HandshakeProtocol.ServerSide.RESETTING);
		context().system()
    	.scheduler()
    	.scheduleOnce(Duration.ofMillis(1000), 
            ...
            public void run() {
            	if (isLoaded) {
            		publishNewState(HandshakeProtocol.ServerSide.IDLE_LOADED);
            	} else {
            		publishNewState(HandshakeProtocol.ServerSide.IDLE_EMPTY);
            	}
            }
	...
	private void initHandover() {
		if (currentState.equals(HandshakeProtocol.ServerSide.IDLE_EMPTY) || currentState.equals(HandshakeProtocol.ServerSide.IDLE_LOADED)) {
			publishNewState(HandshakeProtocol.ServerSide.STARTING);
	    	...
	    	.scheduleOnce(Duration.ofMillis(200), 
	            ...
	            @Override
	            public void run() {
	            	publishNewState(HandshakeProtocol.ServerSide.PREPARING);
	            	if (isLoaded) {	            		
	            		publishNewState(HandshakeProtocol.ServerSide.READY_LOADED);
	            	} else {	            		
	            		publishNewState(HandshakeProtocol.ServerSide.READY_EMPTY);
	            	}
	            }
	          }, context().system().dispatcher());
		} else if (currentState.equals(HandshakeProtocol.ServerSide.READY_EMPTY) || currentState.equals(HandshakeProtocol.ServerSide.READY_LOADED)){
	...
	private void  startHandover() {
		if ((currentState.equals(HandshakeProtocol.ServerSide.READY_EMPTY) || currentState.equals(HandshakeProtocol.ServerSide.READY_LOADED))) {
			publishNewState(HandshakeProtocol.ServerSide.EXECUTE);
		...
		} else if ( currentState.equals(HandshakeProtocol.ServerSide.EXECUTE) ){ //resending
			getSender().tell(HandshakeProtocol.ServerMessageTypes.OkResponseStartHandover, self);
		} else {
	...
	private void complete() {
		publishNewState(HandshakeProtocol.ServerSide.COMPLETING);
		context().system()
    	.scheduler()
    	.scheduleOnce(Duration.ofMillis(1000), 
    			 new Runnable() {
            ...
            public void run() {
            	...
            	isLoaded = !isLoaded;
            	publishNewState(HandshakeProtocol.ServerSide.COMPLETE);
//            	stop(); // we automatically stop --> no longer stop, just remain in complete
	...
	private void stop() {
		publishNewState(HandshakeProtocol.ServerSide.STOPPING);
		//clientSide = null;
		context().system()
    	.scheduler()
    	.scheduleOnce(Duration.ofMillis(1000), 
            ...
            @Override
            public void run() {
            	publishNewState(HandshakeProtocol.ServerSide.STOPPED);
            }
          }, context().system().dispatcher());
	}	
