...
public class OPCUAClientHandshakeActorWrapper extends AbstractActor {
	...
	private void unsubscribeFromStatus() {
		nodeIds.client.getSubscriptionManager().clearSubscriptions();
		isSubscribed = false;
	...
	private CompletableFuture<String> callMethod(NodeId methodId) {
		...
		CallMethodRequest request = new CallMethodRequest(
				nodeIds.capabilityImplNode, methodId, new Variant[]{});

		return nodeIds.client.call(request).thenCompose(result -> {
	...
	private void setNewNodeIds(ServerHandshakeNodeIds nodeIds) {
		if (this.nodeIds != null && this.nodeIds.client != null) {
			try {
				this.nodeIds.client.disconnect().get(2, TimeUnit.SECONDS);
	...
	public void subscribeToStatus() {
		...
		try {
			UaSubscription subscription = nodeIds.client.getSubscriptionManager().createSubscription(100.0).get();
			ReadValueId readValueId = new ReadValueId(nodeIds.stateVar, AttributeId.Value.uid(), null, QualifiedName.NULL_VALUE);
	...
	private void onStateSubscriptionChange(UaMonitoredItem item, DataValue value) {
		...
		if( value.getValue().isNotNull() ) {
			...
			try {
				HandshakeProtocol.ServerSide state = HandshakeProtocol.ServerSide.valueOf(stateAsString);
	...
	public static class ServerHandshakeNodeIds {
		OpcUaClient client;
