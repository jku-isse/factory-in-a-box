...
public class HandshakeFU implements StatePublisher {
	...
	private final Map<String, org.eclipse.milo.opcua.sdk.server.nodes.UaVariableNode> wiringNodes;

	public HandshakeFU(OPCUABase base, UaFolderNode root, String fuPrefix, ActorRef ttBaseActor, ActorContext context,
			String capInstId, boolean isProvided, boolean enableCoordinatorActor) {
		this.base = base;
		this.rootNode = root;
	...
	private void setupOPCUANodeSet() {
		...
		if (isProvided) { // add all the methods available here, actor is the serverhandshake
			status = base.generateStringVariableNode(handshakeNode, path, HandshakeProtocol.STATE_SERVERSIDE_VAR_NAME,
					ServerSide.STOPPED);
			localClient = context.actorOf(MockServerHandshakeActor.props(ttBaseActor, true, this));
			org.eclipse.milo.opcua.sdk.server.nodes.UaMethodNode n1 = base.createPartialMethodNode(path,
					"INIT_HANDOVER", "Requests init");
			base.addMethodNode(handshakeNode, n1, new InitHandover(n1, localClient));
			org.eclipse.milo.opcua.sdk.server.nodes.UaMethodNode n2 = base.createPartialMethodNode(path,
					"START_HANDOVER", "Requests start");
			base.addMethodNode(handshakeNode, n2, new StartHandover(n2, localClient));
			// add set wiring method
			org.eclipse.milo.opcua.sdk.server.nodes.UaMethodNode wiringMethod = base.createPartialMethodNode(path,
					"SET_WIRING", "Sets the wiring Info");
			...
			if (!enableCoordinatorActor) {
				...
				org.eclipse.milo.opcua.sdk.server.nodes.UaMethodNode n3 = base.createPartialMethodNode(path,
						HandshakeProtocol.ServerMessageTypes.Stop.toString(), "Request stop");
				base.addMethodNode(handshakeNode, n3, new Stop(n3, localClient));
				org.eclipse.milo.opcua.sdk.server.nodes.UaMethodNode n4 = base.createPartialMethodNode(path,
						HandshakeProtocol.ServerMessageTypes.Reset.toString(), "Request reset");
				base.addMethodNode(handshakeNode, n4, new Reset(n4, localClient));
				org.eclipse.milo.opcua.sdk.server.nodes.UaMethodNode n5 = base.createPartialMethodNode(path,
						HandshakeProtocol.ServerMessageTypes.Complete.toString(), "Request complete");
				base.addMethodNode(handshakeNode, n5, new Complete(n5, localClient));
				org.eclipse.milo.opcua.sdk.server.nodes.UaMethodNode n6 = base.createPartialMethodNode(path,
						StateOverrideRequests.SetLoaded.toString(), "Request SetLoaded");
				base.addMethodNode(handshakeNode, n6, new SetLoaded(n6, localClient));
				org.eclipse.milo.opcua.sdk.server.nodes.UaMethodNode n7 = base.createPartialMethodNode(path,
						StateOverrideRequests.SetEmpty.toString(), "Request SetEmpty");
				base.addMethodNode(handshakeNode, n7, new SetEmpty(n7, localClient));
			}

			// let parent Actor know, that there is a new endpoint
			ttBaseActor.tell(new LocalEndpointStatus.LocalServerEndpointStatus(localClient, isProvided, this.capInstId),
					ActorRef.noSender());
		} else {
			status = base.generateStringVariableNode(handshakeNode, path, HandshakeProtocol.STATE_CLIENTSIDE_VAR_NAME,
					ClientSide.STOPPED);
			...
			ttBaseActor.tell(new LocalEndpointStatus.LocalClientEndpointStatus(localClient, isProvided, this.capInstId),
					ActorRef.noSender());
			...
			if (!enableCoordinatorActor) {
				...
				org.eclipse.milo.opcua.sdk.server.nodes.UaMethodNode n3 = base.createPartialMethodNode(path,
						HandshakeProtocol.ClientMessageTypes.Stop.toString(), "Request stop");
				base.addMethodNode(handshakeNode, n3,
						new fiab.opcua.hardwaremock.clienthandshake.methods.Stop(n3, localClient));
				org.eclipse.milo.opcua.sdk.server.nodes.UaMethodNode n4 = base.createPartialMethodNode(path,
						HandshakeProtocol.ClientMessageTypes.Reset.toString(), "Request reset");
				base.addMethodNode(handshakeNode, n4,
						new fiab.opcua.hardwaremock.clienthandshake.methods.Reset(n4, localClient));
				org.eclipse.milo.opcua.sdk.server.nodes.UaMethodNode n5 = base.createPartialMethodNode(path,
						HandshakeProtocol.ClientMessageTypes.Complete.toString(), "Request complete");
				base.addMethodNode(handshakeNode, n5,
						new fiab.opcua.hardwaremock.clienthandshake.methods.Complete(n5, localClient));
				org.eclipse.milo.opcua.sdk.server.nodes.UaMethodNode n6 = base.createPartialMethodNode(path,
						HandshakeProtocol.ClientMessageTypes.Start.toString(), "Start complete");
				base.addMethodNode(handshakeNode, n6,
						new fiab.opcua.hardwaremock.clienthandshake.methods.Start(n6, localClient));
			}
		}

		// add capabilities
		UaFolderNode capabilitiesFolder = base.generateFolder(handshakeNode, path,
		...
		UaFolderNode capability1 = base.generateFolder(capabilitiesFolder, path, "CAPABILITY",
				OPCUACapabilitiesWellknownBrowsenames.CAPABILITY);
		...
		base.generateStringVariableNode(capability1, path + "/CAPABILITY", OPCUACapabilitiesWellknownBrowsenames.ROLE,
				new String(isProvided ? OPCUACapabilitiesWellknownBrowsenames.ROLE_VALUE_PROVIDED
						: OPCUACapabilitiesWellknownBrowsenames.ROLE_VALUE_REQUIRED));

		createWiringInfoFolder(handshakeNode, path);
		...
		if (!isProvided) { // only if required endpoint, i.e., this is a client
			...
			if (!emptyWiringInfo(info)) {
				...
				if (optRemoteNodeId.isPresent()) {
					logger.info("Searching for Grandparent Node for Capability: "
							+ optRemoteNodeId.get().toParseableString());
					...
					if (optActorCapImplNodeId.isPresent()) {
						...
						if (!nodeIds.isComplete()) {
							logger.error("OPCUA Client Endpoints incompletely resolved for FU: " + this.capInstId
									+ " at " + optActorCapImplNodeId.get().toParseableString());
						} else {
							...
							setWiringInformation(info.getRemoteCapabilityId(), info.getRemoteEndpointURL(),
									info.getRemoteNodeId(), info.getRemoteRole());
						}
					} else {
	...
	private boolean emptyWiringInfo(WiringInfo info) {
		if (info.localCapabilityId.isEmpty() && info.remoteCapabilityId.isEmpty() && info.remoteEndpointURL.isEmpty()
				&& info.remoteNodeId.isEmpty() && info.remoteRole.isEmpty())
			return true;
		return false;
	}
	...
	private ServerHandshakeNodeIds retrieveNodeIds(CapabilityImplInfo info)
			throws InterruptedException, ExecutionException {
		List<Node> nodes = info.getClient().getAddressSpace().browse(info.getActorNode()).get();
		// we assume unique node names and method names within this hierarchy level
		// (thus no two capabilities with overlapping browse names)
	...
	private Optional<NodeId> getGrandParentForNodeIdViaBrowse(OpcUaClient client, NodeId nodeIdToSearchFor,
			NodeId currentNode, NodeId parent) {
		try {
