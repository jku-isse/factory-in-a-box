...
class TestTransportSystemCoordinatorActor {
	...
	void testCoordinatorWithMinimumLayout() throws InterruptedException, ExecutionException {
		new TestKit(system) { 
			{ 															
				...
				equals(dns.getActorForPosition(new Position("20")).equals(knownActors.get("MockTurntableActor20")));
				RegisterTransportRequest rtr = new RegisterTransportRequest(knownActors.get("InputStationActor34"), knownActors.get("OutputStationActor35"), "TestOrder1");
	...
	private void setupMinimalShopfloor() throws InterruptedException, ExecutionException {
		final ActorSelection eventBusByRef = system.actorSelection("/user/"+InterMachineEventBusWrapperActor.WRAPPER_ACTOR_LOOKUP_NAME);
		
		partsIn = MockIOStationFactory.getMockedInputStation(system, eventBusByRef, disengageAutoReload);
		partsOut = MockIOStationFactory.getMockedOutputStation(system, eventBusByRef, disengageAutoReload);
