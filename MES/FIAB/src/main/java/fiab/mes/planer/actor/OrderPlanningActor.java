...
public class OrderPlanningActor extends AbstractTracingActor {
	...
	// protected List<MappedOrderProcess> orders = new
	// ArrayList<MappedOrderProcess>();
	...
	// protected Map<ActorRef, List<AbstractCapability>> capabilities = new
	// HashMap<>();
	// protected Map<Actor, ActorRef> modelActors2AkkaActors = new HashMap<>();
	// dont make this bidirectional as we use them differently!!
	// protected Map<ActorRef, String> machineWorksOnOrder = new HashMap<ActorRef,
	// String>();
	// protected Map<String, ActorRef> rootOrderAllocatedToMachine = new
	// HashMap<String, ActorRef>();
	// protected Map<Entry<AkkaActorBackedCoreModelAbstractActor, String>,
	// OrderEventType> scheduleStatus = new HashMap<>();
	...
	public OrderPlanningActor() {
		...
		try {
			...
			this.context().parent().tell(new PlanerStatusMessage(WELLKNOWN_LOOKUP_NAME, MachineEventType.INITIALIZING,
					state, "Failed to start due to: " + e.getMessage()), this.self());
		}
			...
			try {
				...
				if (state.equals(PlannerState.FULLY_OPERATIONAL)) {
					...
					String msg = String.format(
							...
							rpReq.getRootOrderId(), PlannerState.FULLY_OPERATIONAL, state);
					log.warning(msg);
	...
	private void getEventBusAndSubscribe() throws Exception {
		SubscribeMessage orderSub = new SubscribeMessage(getSelf(),
				new MESSubscriptionClassifier(WELLKNOWN_LOOKUP_NAME, "*"));
		...
		SubscribeMessage machineSub = new SubscribeMessage(getSelf(),
				new MESSubscriptionClassifier(WELLKNOWN_LOOKUP_NAME, "*"));
		machineEventBus = this.context()
				.actorSelection("/user/" + InterMachineEventBusWrapperActor.WRAPPER_ACTOR_LOOKUP_NAME);
	...
	private void getTransportSystemCoordinator() throws Exception {
		transportCoordinator = this.context()
...
//	}
	...
	private void scheduleProcess(String rootOrderId, OrderProcess mop) {
		if (!mop.doAllLeafNodeStepsHaveInvokedCapability(mop.getProcess())) {
			...
			orderEventBus.tell(new OrderEvent(rootOrderId, this.self().path().name(), OrderEventType.REJECTED, msg),
					ActorRef.noSender());
		} else {
			// we have capability invocations with capabilities, thus able to map them to
			// machines
			ProcessChangeImpact pci = mop.activateProcess();
			String msg = "Capability invocations have capabilities, thus able to map them to machines. rootOrderId: "
					+ rootOrderId;
	...
	// this method will trigger order to machine allocation when there is a machine
	// already idle,
	...
	private void tryAssignExecutingMachineForOneProcessStep(OrderProcess op, String rootOrderId) {
		...
		List<CapabilityInvocation> stepCandidates = op.getAvailableSteps().stream()
				.filter(step -> (step instanceof CapabilityInvocation)).map(CapabilityInvocation.class::cast)
				.filter(capInv -> capInv.getInvokedCapability() != null).collect(Collectors.toList());
		if (stepCandidates.isEmpty()) {
			...
			if (op.areAllTasksCancelledOrCompleted()) {
				// no need to signal completion here, as step level completion already
				// happended, and completion only when process arrives at output station
				log.info(String.format("OrderProcess %s is complete, now triggering move to output station",
						rootOrderId));
				// transport it off to output station
				ordMapper.markOrderCompleted(rootOrderId,
						"Order about to be moved to next available/idle output station");
				...
				if (!outputStation.isPresent()) {
					// if not possible we wont do anything with this order at the moment and wait
					// for outputstation to become available
				...
				// capabilityinvocation ie. process step to allocate to a machine
				String msg = String.format(
						"OrderProcess %s has no available steps of type CapabilityInvocation to continue with, moving to next available/idle output station",
						rootOrderId);
				log.warning(msg);
				ordMapper.markOrderCanceled(rootOrderId, msg);
				// if this process is somewhere on some machine, remove it from production -->
				// transport it off to output station
				Optional<AkkaActorBackedCoreModelAbstractActor> outputStation = tryReserveOutputStation(rootOrderId);
				if (!outputStation.isPresent()) {
					// if not possible we wont do anything with this order at the moment and wait
			...
			// go through each candidate and check if any of the steps can be mapped to a
			// available machine,
			// first get candidates where the machine capabilites matches
			Map<CapabilityInvocation, Set<AkkaActorBackedCoreModelAbstractActor>> capMap = stepCandidates.stream().map(
					step -> new AbstractMap.SimpleEntry<CapabilityInvocation, Set<AkkaActorBackedCoreModelAbstractActor>>(
							step, capMan.getMachinesProvidingCapability(step.getInvokedCapability())))
					.filter(pair -> !pair.getValue().isEmpty()).collect(Collectors.toMap(Map.Entry::getKey,
							Map.Entry::getValue, (prev, next) -> next, HashMap::new));

			if (capMap.isEmpty()) {
				log.warning(String.format(
						"OrderProcess %s has no available CapabilityInvocations steps match a discovered Machine/Actor, Order is paused",
						rootOrderId));
				ordMapper.pauseOrder(rootOrderId);
			} else { // among those with available capabilities, select first where there is a
						// machine mapped and available at the moment, implement JIT assignment

				// for each step produce tuple list of available machines that match the
				// capability required
				Optional<AbstractMap.SimpleEntry<CapabilityInvocation, AkkaActorBackedCoreModelAbstractActor>> maOpt = capMap
						.entrySet().stream().flatMap(pair -> pair.getValue().stream()
								.map(mach -> new AbstractMap.SimpleEntry<>(pair.getKey(), mach)))
						.filter(flatPair -> {
						...
						.filter(pair -> {
							return ordMapper.getMappingStatusOfMachine(pair.getValue())
									.orElse(new MachineOrderMappingStatus(pair.getValue(), AssignmentState.UNKNOWN))
									.getAssignmentState().equals(AssignmentState.NONE); // and also where we havent
																						// assigned any order to yet
						}).findFirst(); // then from this list pick the first

				maOpt.ifPresent(aa -> {
					// scheduled, not yet producing
					Optional<AkkaActorBackedCoreModelAbstractActor> currentLoc = ordMapper
							.getCurrentMachineOfOrder(rootOrderId);
					if (!currentLoc.isPresent()) {
//						assign inputstation 
						Optional<AkkaActorBackedCoreModelAbstractActor> inputStation = tryReserveNewProcessAtInputStation(
								rootOrderId);
						if (!inputStation.isPresent()) {
							// if not possible we wont do anything with this order at the moment and wait
							// for inputstation to become available
							log.info(String.format(
									"Order %s and potential (not yet requested) machine %s waiting for next available inputstation",
									rootOrderId, aa.getValue().getId()));
							return;
						}
					}
					...
					// TODO: alternatively the machine could issue a SCHEDULED event (BUT WE NEED TO
					// TRACK OUR REQUESTS)
				...
				if (!maOpt.isPresent()) {
					log.info(String.format(
							"OrderProcess %s has no available CapabilityInvocations steps that match an available Machine/Actor, Order is paused",
							rootOrderId));
					ordMapper.pauseOrder(rootOrderId);
				}
			}
	...
	private Optional<AkkaActorBackedCoreModelAbstractActor> tryReserveNewProcessAtInputStation(String orderId) {
		...
		Optional<AkkaActorBackedCoreModelAbstractActor> availInput = iStations.stream()
				.filter(io -> ordMapper.getMappingStatusOfMachine(io)
						.orElse(new MachineOrderMappingStatus(io, AssignmentState.UNKNOWN)).getAssignmentState()
						.equals(AssignmentState.NONE))
				.findAny();
		return availInput.map(iStation -> {
			try {
				ordMapper.requestMachineForOrder(iStation, orderId, null); // null JobId as we request transport
				ordMapper.confirmOrderAtMachine(iStation); // and we occupy this machine immediately, so we can "locate"
															// it when requesting transport
			} catch (MachineOrderMappingStatusLifecycleException e) {
				e.printStackTrace();
				return null;
			}
	...
	private void produceProcessAtMachine(ReadyForProcessEvent readyE) {
		...
		// TODO: if order is canceled already, freeup has happened, machine has been
		// told
		...
		capMan.resolveByAkkaActor(getSender()).ifPresent(machine -> {
			if (readyE.isReady()) {
				...
				ordMapper.getOrderRequest(orderId).ifPresent(rpr -> {
					ProcessChangeImpact pci = rpr.getProcess()
							.activateStepIfAllowed(readyE.getResponseTo().getProcessStep());
					String msg = String.format("Machine with ID %s agreed to work on Order with ID %s", machine.getId(),
							orderId);
					log.info(msg);
					orderEventBus.tell(new OrderProcessUpdateEvent(orderId, this.self().path().name(), msg, pci),
							ActorRef.noSender());
				});

			} else { // e.g., when machine needs to go down for maintenance, or some other error
	...
	private void handleIOStationUpdateEvent(IOStationStatusUpdateEvent ue) {
		log.info(String.format("IOStationUpdateEvent for machine %s : %s", ue.getMachineId(),
				ue.getStatus().toString()));
	...
	private void handleCancelOrderRequest(CancelOrTerminateOrder req) {
		...
		ordMapper.getOrderRequest(req.getRootOrderId()).ifPresent(oreq -> {
			...
			String msg = String.format("Request to cancel Order %s - canceling all remaining steps",
					req.getRootOrderId());
			log.info(msg);
		...
		ordMapper.getLastOrderState(req.getRootOrderId()).ifPresent(oStatus -> {
			// transient states exist only briefly and thus should never be set when we
			// enter here
			switch (oStatus) {

			case PRODUCING: // fallthrough
				// when machine finishes, we override automatically the step as completed, but
				// all other steps will be canceled thus order will be transported off to
				// outputstation
			case COMPLETED:
				// process is waiting for output station, nothing to do here, no point in
				// further canceling
			...
			case SCHEDULED:
				...
				ordMapper.getRequestedMachineOfOrder(req.getRootOrderId()).ifPresent(machine -> {
					machine.getAkkaActor().tell(cot, self);
				});
				// then remove from current machine
				// via fallthrough
			case PAUSED: // at machine or before first machine
				Optional<AkkaActorBackedCoreModelAbstractActor> currentLoc = ordMapper
						.getCurrentMachineOfOrder(req.getRootOrderId());
				if (!currentLoc.isPresent()) {
					String msg = String.format("Request to cancel Order %s confirmed before order entered shopfloor",
							req.getRootOrderId());
					log.info(msg);
					ordMapper.markOrderRejected(req.getRootOrderId(), msg);
				} else { // order at machine
					String msg = String.format(
							"Request to cancel Order %s confirmed while pause at machine, will be transported to output station",
							req.getRootOrderId());
					log.info(msg);
					ordMapper.markOrderCanceled(req.getRootOrderId(), msg);
					Optional<AkkaActorBackedCoreModelAbstractActor> outputStation = tryReserveOutputStation(
							req.getRootOrderId());
					if (!outputStation.isPresent()) {
						// if not possible we wont do anything with this order at the moment and wait
						// for outputstation to become available
			...
			case TRANSPORT_IN_PROGRESS: // cancel at receiving machine,
				String msg = String.format(
						"Request to cancel Order %s confirmed while order waiting for transport or in transit",
						req.getRootOrderId());
				log.info(msg);
				ordMapper.markOrderCanceled(req.getRootOrderId(), msg);
				ordMapper.getRequestedMachineOfOrder(req.getRootOrderId()).ifPresent(machine -> {
			...
			case CANCELED: // transient state
				log.error("Encountered order in transient state " + oStatus
						+ " when trying to cancel or terminate, ignoring request");
				break;
	...
	private void handleMachineUpdateEvent(MachineStatusUpdateEvent mue) {
		log.info(String.format("MachineUpdateEvent for machine %s : %s", mue.getMachineId(),
				mue.getStatus().toString()));
		capMan.resolveById(mue.getMachineId()).ifPresent(machine -> {
			...
			case COMPLETING:
				...
				// we cannot rely on ProductionCompletionevent as there could be a race
				// condition which event arrives first

				// then we still have the order occupying the machine --> now check if next
				// place is ready somewhere
				// update process, get next step --> check if possible somewhere
				ordMapper.getOrderRequestOnMachine(machine).ifPresent(rpr -> {
					log.debug(String.format("%s in state %s with register process request of %s", machine.getId(),
							mue.getStatus().toString(), rpr.getRootOrderId()));
					ordMapper.getJobOnMachine(machine).ifPresent(step -> {
						ProcessChangeImpact pci = rpr.getProcess().markStepComplete(step);
						String msg = String.format(
								"Update process that step %s is complete at machine with ID %s and with order ID %s",
								step.getDisplayName(), machine.getId(), rpr.getRootOrderId());
						log.info(msg);
						OrderProcessUpdateEvent opue = new OrderProcessUpdateEvent(rpr.getRootOrderId(),
	...
	private void handleStoppingOrStoppedMachine(AkkaActorBackedCoreModelAbstractActor machine) {
		// this is called when the machine is explicitly stopped, e.g. to shut it down
		...
		ordMapper.getMappingStatusOfMachine(machine.getId()).ifPresent(moms -> {
			...
			case NONE:
				break;
			case REQUESTED: // we asked machine but it has not responded, thus order still in previous
							// location
				log.info(String.format("Freeing up STOPPING/STOPPED Machine %s originally REQUESTED for order %s ",
						machine.getId(), moms.getOrderId()));
				ordMapper.pauseOrder(moms.getOrderId());
				ordMapper.freeUpMachine(machine, true);
				break;
			case RESERVED: // we got a positive response to work and perhaps asked for transport
				log.info(String.format("Freeing up STOPPING/STOPPED Machine %s originally RESERVED for order %s ",
						machine.getId(), moms.getOrderId()));
				// now we need to check if order is (about to be) in transit
				ordMapper.getLastOrderState(moms.getOrderId()).ifPresent(state2 -> {
					log.info(String.format("Checking potential Transport Progress state %s for RESERVED order %s ",
							state2, moms.getOrderId()));
					if (state2.equals(OrderEventType.TRANSPORT_REQUESTED)
				...
				ordMapper.freeUpMachine(machine, true);
				break;
			case OCCUPIED: // the machine is working on this order, cancel Order, then manual removal
							// needed
				// order might completed and about to be in transit
				ordMapper.getLastOrderState(moms.getOrderId()).ifPresent(state -> {
					log.info(String.format("Checking potential Transport Progress state %s for OCCUPIED order %s ",
							state, moms.getOrderId()));
				...
				ordMapper.markOrderPrematureRemovalFromShopfloor(moms.getOrderId(),
						"Machine stopped unexpectedly, please manally remove order from machine: " + machine.getId());
				break;
			default:
				break;
			}
		}); // if not present, then nothing to do
		// if this is called a second time (after stopping) then also in stopped,
		// nothing will happen as the ordMapper has set the machine to Unknown, or None
	}
	...
	private void handleNewlyAvailableMachine(MachineConnectedEvent mce) {
		...
		checkPlannerState();
		// now wait for machine available event to make use of it (currently we dont
		// know its state)
	}
	...
	private void checkPlannerState() {
		...
		if (newState != currState) {
			...
			publishLocalState(MachineEventType.UPDATED, state,
					"Input/Outputstation/Transportsystem availability changed");
		}
	}
	...
	private void requestTransport(AkkaActorBackedCoreModelAbstractActor destination, String orderId) {
		...
		if (!currentLoc.isPresent()) {
			...
			log.error(msg);
			// now this is really an error as we ensure when requesting a machine that we do
			// this only when first reserving and allocating an inputstation
			ordMapper.markOrderPrematureRemovalFromShopfloor(orderId, msg);
			// free up destination
			ordMapper.freeUpMachine(destination, true);
			// TODO: if outputstation then dont need a new event, depending where the
			// request failed, perhaps already at new machine?!
			// TODO: if source is inputstation, then freeup also input station from
			// reservation
		} else {
	...
	private void handleResponseToTransportRequest(RegisterTransportRequestStatusResponse resp) {
		...
		case COMPLETED:
			ordMapper.getLastOrderState(orderId).ifPresent(state -> {
				if (state.equals(OrderEventType.CANCELED)) { // order at new machine, remove there and reset machine
					String msg2 = String.format(
							"Order was canceled please remove manually from destination machine and reset destination machine");
					log.warning(msg2);
			...
			capMan.getMachinesProvidingCapability(outputStationCap).stream().filter(machine -> machine.equals(machine2))
					.findAny().ifPresent(oStation -> { // an output station
						ordMapper.freeUpMachine(machine2, false);
						ordMapper.markOrderRemovedFromShopfloor(orderId,
								"Order left the shopfloor via Outputmachine: " + oStation.getId());
					});
		...
		case FAILED_IN_TRANSPORT:
			msg = String.format("Transport of Order %s failed with reason %s : %s", orderId, resp.getResponse(),
					resp.getMessage());
			ordMapper.markOrderFailedTransport(orderId, msg);
			ordMapper.markOrderPrematureRemovalFromShopfloor(orderId, msg);
			log.warning(msg);
			// we need to manually remove the pallet and reset the transport modules,
			...
			machine1.getAkkaActor().tell(new CancelOrTerminateOrder(resp.getOriginalRequest().getOrderId(), jobId1),
					self);
			break;
		...
		case UNSUPPORTED_TRANSIT_POSITION: // fallthrough
			...
			ordMapper.markOrderPrematureRemovalFromShopfloor(orderId, msg);
			log.warning(msg);
			// we need to manually remove the pallet from machine and reset the machine,
			// nothing we can handle here.
			...
			machine.getAkkaActor().tell(new CancelOrTerminateOrder(resp.getOriginalRequest().getOrderId(), jobId),
					self);
			break;
		case QUEUED:
		...
		case ISSUED:
			...
			break;
		case CANCELED:
			// transport did not happen, request is removed, pallet still at current
			// location, we need to reset state to paused
			...
			break;
		case ABORTED:
			// transport was already under way, pallet needs to be removed manually from
			// current locaton, which may be between machine and turntable
			msg = String.format("Transport was aborted, please remove manually from turntable");
			log.warning(msg);
			ordMapper.markOrderPrematureRemovalFromShopfloor(orderId, msg);
			// TODO: we could have just managed to transport way from a stopped machine,
			// then we would not need to do this, but this needs to be handled in the
	...
	// manage a list of loading and unloading stations (at least one each needs to
	// be available)
